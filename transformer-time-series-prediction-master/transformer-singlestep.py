...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        super(PositionalEncoding, self).__init__()        # d_model token维度
        pe = torch.zeros(max_len, d_model)  # 位置向量
...
class TransAm(nn.Module):
    def __init__(self, feature_size=250, num_layers=1, dropout=0.1):
        ...
        self.input_embedding = nn.Linear(1, feature_size)  # 输入1维，输出feature_size维
        self.src_mask = None
...
def get_data():
    ...
    #
    df = pandas.read_csv(
        r"C:\\Users\\MOIGE\\Desktop\\实习\\air+quality\\AirQualityUCI.csv",
        header=0,
        sep=';',
        encoding='utf-8',
        usecols=['Date', 'Time', 'CO(GT)', "PT08.S1(CO)", 'NMHC(GT)', 'C6H6(GT)', 'PT08.S2(NMHC)', 'NOx(GT)',
    ...
    df['Datetime'] = pandas.to_datetime(df['Date'] + ' ' + df['Time'], format='%d/%m/%Y %H.%M.%S')
    # 将'Datetime'列转换为Unix时间戳
    df['UnixTime'] = pandas.to_numeric(df['Datetime']) / 10 ** 9
    ...
    df = df.astype(float)
    df = df.to_numpy()
    series = scaler.fit_transform(df.reshape(-1, 1)).reshape(-1)  # -1表示自适应，1表示1列
    sampels = int(int(df.shape[0]) * train_size)  # use a parameter to control training size
    train_data = series[:sampels]
    test_data = series[sampels:]

    # train_np = df.iloc[:int(df.shape[0]*0.7), :].values
    # test_np = df.iloc[int(df.shape[0]*0.7):, :].values
...
def train(train_data):
    ...
    for batch, i in enumerate(range(0, len(train_data), batch_size)):  # Now len-1 is not necessary
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 0.7)  # 梯度裁剪
        optimizer.step()

        total_loss += loss.item()
        log_interval = int(len(train_data) / batch_size / 5)  # log间隙
        if batch % log_interval == 0 and batch > 0:
            cur_loss = total_loss / log_interval
            elapsed = time.time() - start_time   # 消耗时间
            print('| epoch {:3d} | {:5d}/{:5d} batches | '
                  'lr {:02.6f} | {:5.2f} ms | '
                  'loss {:5.5f} | ppl {:8.2f}'.format(
                    epoch, batch, len(train_data) // batch_size, scheduler.get_lr()[0],  # get_last_lr()
