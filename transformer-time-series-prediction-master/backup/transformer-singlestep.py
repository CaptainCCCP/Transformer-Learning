...
from new.dataloader import train_dataloder, val_dataloder
torch.manual_seed(0)
np.random.seed(0)
...
def get_data():
    ...
    time = np.arange(0, len(series)/10 , 0.1)

    from sklearn.preprocessing import MinMaxScaler
    scaler = MinMaxScaler(feature_range=(-1, 1))

    series = series.to_numpy()
    series = scaler.fit_transform(series.reshape(-1, 1)).reshape(-1)  # -1表示自适应，1表示1列
